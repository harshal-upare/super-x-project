...
public class Earnings {
    ...
    public static class Transaction {
        ...
        public Transaction(String txnId, String title, String date, String channel, int amount, String type, String status) {
            this.txnId = txnId;
            this.title = title;
            this.date = date;
    ...
    private static void initTransactions() {
        ...
        txnList.add(new Transaction("#TXN-8921", "Harvester Rental Payout (Balasaheb Shirole)", "14 Aug 2026", "Direct Bank IMPS", 12600, "CREDIT", "SETTLED"));
        txnList.add(new Transaction("#TXN-8894", "Rotavator 3-Day Job Payout (Vikas More)", "13 Aug 2026", "Direct Bank IMPS", 2280, "CREDIT", "SETTLED"));
        txnList.add(new Transaction("#WD-5021", "Provider Bank Withdrawal to HDFC Bank •••• 8842", "11 Aug 2026", "IMPS Transfer", 25000, "WITHDRAWAL", "SETTLED"));
        txnList.add(new Transaction("#TXN-8850", "Tractor 5-Day Rental Payout (Ganesh Jadhav)", "10 Aug 2026", "Direct Bank IMPS", 7125, "CREDIT", "SETTLED"));
        txnList.add(new Transaction("#TXN-8790", "Drone Spraying Service Payout (Kiran Bhosale)", "05 Aug 2026", "Direct Bank IMPS", 1710, "CREDIT", "SETTLED"));
        txnList.add(new Transaction("#WD-4980", "Provider Bank Withdrawal to SBI •••• 4120", "01 Aug 2026", "NEFT Transfer", 40000, "WITHDRAWAL", "SETTLED"));
    }
    ...
    public static ScrollPane getEarningsSection(StackPane root) {
        ...
        Text headerSubtitle = new Text("Monitor your machine rental earnings, review escrow settlements, and initiate instant withdrawals to your bank.");
        ...
        // Revenue by Equipment Breakdown Card
        VBox revenueBreakdownCard = createRevenueBreakdown();

        // Transaction History Table Section
        VBox txnSection = createTransactionHistorySection(root);

        VBox content = new VBox(22, topBar, metricRow, revenueBreakdownCard, txnSection);
    ...
    private static HBox createFinancialMetrics() {
        VBox c1 = createMetricCard("💰 Lifetime Earnings", "₹" + String.format("%,d", totalLifetime), "From 142 completed rentals", "#4A2C20");
        ...
        VBox c3 = createMetricCard("⏱ Pending Escrow", "₹" + String.format("%,d", pendingEscrow), "Releases upon job completion", "#E65100");
        ...
        VBox c4 = createCustomMetricCard("💳 Total Settled to Bank", withdrawnText, "18 withdrawals processed", "#5C4033");
    ...
    private static VBox createRevenueBreakdown() {
        Text title = new Text("Machinery Revenue Contribution");
        title.setStyle("-fx-font-family: 'Poppins'; -fx-font-size: 17px; -fx-font-weight: bold; -fx-fill: #4A2C20;");

        VBox r1 = createProgressRow("🌾 Combine Harvesters", "₹2,17,000 (45%)", 0.45, "#8B6F47");
        VBox r2 = createProgressRow("🚜 Tractors (Mahindra & John Deere)", "₹1,55,000 (32%)", 0.32, "#2E7D32");
        VBox r3 = createProgressRow("⚙ Rotavators & Tillers", "₹68,000 (14%)", 0.14, "#E65100");
        VBox r4 = createProgressRow("🚁 Agri Sprayer Drones", "₹45,200 (9%)", 0.09, "#1976D2");
    ...
    private static void showWithdrawModal(StackPane root) {
        ...
        Text avail = new Text("Available Balance for Withdrawal: ₹" + String.format("%,d", availableBalance));
        ...
        modeSelect.getItems().addAll("Instant IMPS (Immediate Credit)", "NEFT Transfer (Within 2 Hours)", "Direct UPI to PhonePe / GPay");
        ...
        submitBtn.setOnAction(e -> {
            try {
                ...
                if (amt > 0 && amt <= availableBalance) {
                    ...
                    txnList.add(0, new Transaction("#WD-" + (5050 + txnList.size()), "Withdrawal to " + bankSelect.getValue().substring(0, 9), "Today (Instant)", modeSelect.getValue().substring(0, 12), amt, "WITHDRAWAL", "SETTLED"));
                    renderTxnList();
